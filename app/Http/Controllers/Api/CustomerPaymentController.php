...
class CustomerPaymentController extends Controller
{
    ...
    {
        $query = CustomerPayment::query()->with('customer', 'paymentMethod', 'bank')->orderBy('date', 'desc');
    ...
    public function store(Request $request)
    {
        ...
        if ($paymentMethod->slug == 'bank-transfer' || $paymentMethod->slug == 'payment-advance') {
            if ($request->bank) {
                ...
                $newTransaction->customer_id = $request->customer_id;
    ...
    {
        $payments = CustomerPayment::whereIn('id', $request->ids)->get();
