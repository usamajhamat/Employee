<template>
  ...
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-gradient-to-r from-teal-600 to-cyan-600 px-6 py-4 rounded-t-lg">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center">
              ...
              <div>
                <h1 class="text-xl font-semibold text-white">Company List</h1>
                <p class="text-teal-100 mt-1">Manage and view all companies</p>
              </div>
            </div>
      ...
      <!-- Empty State -->
      <div
        v-else-if="companiesData?.data?.length === 0"
        class="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center"
      >
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6 px-6 py-4">
          <div class="flex items-center justify-between">
            ...
            <div class="text-sm text-gray-500">
              Showing {{ companiesData?.from || 0 }} to {{ companiesData?.to || 0 }} of
              {{ companiesData?.total || 0 }} results
            </div>
          </div>
        </div>
                ...
                <div class="relative">
                  ...
                  >
                    <div class="py-1">
                      ...
                      <router-link
                        :to="{ name: 'EditCompany', query: { companyId: company.id } }"
                      >
                        <button
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex-1 flex justify-between sm:hidden">
              ...
              <button
                @click="navigate(companiesData?.next_page_url)"
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p class="text-sm text-gray-700">
                  Showing
                  <span class="font-medium">{{ companiesData?.from || 0 }}</span>
                  to
                  <span class="font-medium">{{ companiesData?.to || 0 }}</span>
                  of
                  <span class="font-medium">{{ companiesData?.total || 0 }}</span>
                  results
                </p>
              </div>
              <div>
                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                  ...
                  <button
                    v-for="link in companiesData?.links"
                    :key="link.label"
    ...
    >
      ...
      >
        <div class="relative">
          ...
          <!-- Modal Header -->
          <div class="bg-gradient-to-r from-teal-600 to-cyan-600 p-6 rounded-t-xl text-white shadow-sm">
            <div class="flex items-center space-x-4">
              <div class="h-12 w-12 rounded-lg bg-white bg-opacity-20 flex items-center justify-center">
                <Building2 class="h-6 w-6 text-white" />
              </div>
              <div>
          ...
          <div v-if="selectedCompany" class="p-6 space-y-6">
            <div class="grid grid-cols-1 gap-6">
              ...
              <div>
                ...
                <div class="bg-gray-50 rounded-lg px-4 py-3 text-gray-900 text-sm border border-gray-200">
...
const deleteCompany = async (companyId) => {
  try {
    await store.dispatch("company/" + DELETE_COMPANY, { companyId });
    loadCompanies(); // Refresh the list after deletion
  } catch (error) {
    console.error("Error deleting company:", error);
    // Assuming toast is defined elsewhere
    toast("Failed to delete company. Please try again.", { type: "error" });
  }
};
...
const exportCompanies = () => {
  ...
  const csvContent = [
    headers.join(","),
    ...companiesData.value?.data?.map((company) =>
      [`"${company.company_name}"`, `"${company.description || ""}"`, company.created_at || ""].join(",")
    ),
  ].join("\n");
...
const toggleDropdown = (companyId) => {
  activeDropdown.value = activeDropdown.value === companyId ? null : companyId;
};
