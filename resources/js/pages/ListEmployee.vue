    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-gradient-to-r from-teal-600 to-cyan-600 px-6 py-4 rounded-t-lg">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center">
              ...
              <div>
                <h1 class="text-xl font-semibold text-white">Employee List</h1>
                <p class="text-teal-100 mt-1">Manage and view all employees</p>
              </div>
            </div>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6 p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <!-- Company Filter -->
          <div>
            <label for="company-filter" class="block text-sm font-medium text-gray-700 mb-1">
              Filter by Company
            </label>
            <select
              id="company-filter"
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        <!-- Empty State -->
        <div v-else-if="employeesData?.data?.length === 0" class="p-8 text-center">
          <Users class="h-12 w-12 text-gray-400 mx-auto mb-4" />
          <h3 class="text-lg font-medium text-gray-900 mb-2">No employees found</h3>
          <p class="text-gray-500 mb-4">
            {{ searchQuery || selectedCompany || selectedStatus ? "Try adjusting your filters" : "Get started by adding your first employee" }}
          </p>
          <router-link
            :to="{ name: 'AddEmployee' }"
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              >
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-900">
                    {{ employee.join_company ? formatDate(employee.join_company) : "N/A" }}
                  </div>
                </td>

                <!-- Status -->
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full" :class="getStatusClass(employee.status)">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div class="flex items-center justify-end space-x-2">
                    ...
                    <router-link :to="{ name: 'EditEmployee', query: { id: employee.id } }">
                      <button
        ...
        <!-- Backend Pagination -->
        <div v-if="employeesData?.data?.length > 0" class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
          <div class="flex items-center justify-between">
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p class="text-sm text-gray-700">
                  Showing
                  <span class="font-medium">{{ employeesData?.from || 0 }}</span>
                  to
                  <span class="font-medium">{{ employeesData?.to || 0 }}</span>
                  of
                  <span class="font-medium">{{ employeesData?.total || 0 }}</span>
                  results
                </p>
              </div>
              <div>
        ...
        <div class="relative">
          ...
          <div v-if="selectedEmployee" class="space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-600">Candidate ID</label>
                <p class="mt-1 text-base text-gray-900 font-medium">
              ...
              <div>
                ...
                <p class="mt-1 text-base text-gray-900 font-medium">
                  {{ selectedEmployee?.company || "Not Assigned" }}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p class="mt-1 text-base text-gray-900 font-medium">
                  {{ selectedEmployee?.dob ? formatDate(selectedEmployee?.dob) : "N/A" }}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p class="mt-1 text-base text-gray-900 font-medium">
                  {{ selectedEmployee?.join_company ? formatDate(selectedEmployee?.join_company) : "N/A" }}
                </p>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-600">Bank Account Number</label>
                <p class="mt-1 text-base text-gray-900 font-medium">
                  {{ selectedEmployee?.bank_account || "N/A" }}
                </p>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-600">Residance</label>
                <p class="mt-1 text-base text-gray-900 font-medium">
                  {{ selectedEmployee?.residance || "N/A" }}
                </p>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-600">Join Accommodation Date</label>
                <p class="mt-1 text-base text-gray-900 font-medium">
                  {{ selectedEmployee?.join_accommodation ? formatDate(selectedEmployee?.join_accommodation) : "N/A" }}
                </p>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-600">Exit Accommodation Date</label>
                <p class="mt-1 text-base text-gray-900 font-medium">
                  {{ selectedEmployee?.exit_accommodation ? formatDate(selectedEmployee?.exit_accommodation) : "N/A" }}
                </p>
              </div>
            </div>
...
import { ChevronLeft, ChevronRight, Download, Edit, Eye, RotateCcw, Search, Trash2, UserPlus, Users, X } from "lucide-vue-next";
...
const deleteEmployee = async (employeeId) => {
  try {
    ...
    toast("Failed to delete employee. Please try again.", { type: "error" });
  }
};
...
const exportEmployees = () => {
  const headers = ["Name", "Candidate ID", "Company", "Contact", "Status", "Join Date"];
  const csvContent = [
    headers.join(","),
