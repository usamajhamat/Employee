<template>
    ...
    <div class="min-h-screen bg-gray-50 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-teal-100 mt-1">
                                    Day-wise recruitment analytics (Monday -
                                    Friday)
                                </p>
                            </div>
                        </div>
            ...
            <!-- Filters -->
            <div
                class="bg-white rounded-xl shadow-sm border border-gray-200 mb-8 p-6"
            >
            ...
            <!-- Bar Charts Section -->
            <div
                class="bg-white rounded-xl shadow-sm border border-gray-200 mb-8 p-8"
                data-section="bar"
            >
                <div
                ...
                >
                    <h3 class="text-xl font-semibold text-gray-900">
                        Daily Performance (Monday - Friday)
                    </h3>
                    <div
                        class="mt-4 sm:mt-0 flex flex-wrap items-center space-x-3"
                    ></div>
                </div>
                <div
                    ...
                    >
                        ...
                        <div class="relative h-64 mb-4">
                            <canvas
                                :ref="
                                    (el) =>
                                        setCanvasRef(el, company.name, 'bar')
                                "
                            ></canvas>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <div class="relative h-64 mb-4">
                            <canvas
                                :ref="
                                    (el) =>
                                        setCanvasRef(el, company.name, 'pie')
                                "
                            ></canvas>
                        </div>
                        <div class="space-y-2 text-sm">
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    <div
                                        class="w-3 h-3 bg-purple-500 rounded mr-2"
                                    ></div>
                    ...
                    <div>
                        ...
                        <div class="border-b-2 border-gray-300 pb-2">
                            ...
                            <p id="checked-by-help" class="sr-only">
                                Enter the name of the person who checked the
                                report
                            </p>
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="border-b-2 border-gray-300 pb-2">
                            ...
                            <p id="approved-by-help" class="sr-only">
                                Enter the name of the person who approved the
                                report
                            </p>
                        </div>
                    </div>
...
const companies = computed(() => {
  let data = store.getters["weeklyInfo/weeklyReports"];
  // Ensure data is an array, fallback to empty array if not
  if (!Array.isArray(data)) {
    console.warn("weeklyReports is not an array:", data);
    data = [];
  }
  return data.map((company) => {
    ...
    Object.values(company.dailyData || {}).forEach((day) => {
      ...
      totals.interviewed += day.interviewed || 0;
      totals.passed += day.passed || 0;
      totals.hostel += day.hostel || 0;
      totals.walkin += day.walkin || 0;
    });
...
const fetchWeeklyReports = async () => {
  try {
    await store.dispatch("weeklyInfo/" + FETCH_WEEKLY_REPORTS, {
      week: selectedWeek.value,
    });
    // Log the fetched data for debugging
    console.log("Fetched weeklyReports:", store.getters["weeklyInfo/weeklyReports"]);
  } catch (err) {
  ...
  chartInstances[`bar-${company.name}`] = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Walk-in",
          data: weekDays.map((d) => company.dailyData[d]?.walkin || 0),
          backgroundColor: "#a78bfa",
          borderRadius: 2,
        },
        {
          label: "Hostel",
          data: weekDays.map((d) => company.dailyData[d]?.hostel || 0),
          backgroundColor: "#f59e0b",
          borderRadius: 2,
        },
        {
          label: "Passed",
          data: weekDays.map((d) => company.dailyData[d]?.passed || 0),
          backgroundColor: "#10b981",
          borderRadius: 2,
        },
        {
          label: "Interviewed",
          data: weekDays.map((d) => company.dailyData[d]?.interviewed || 0),
...
const initializeCharts = async () => {
  ...
  if (!filteredCompanies.value || filteredCompanies.value.length === 0) {
    console.warn("No companies data to initialize charts");
    return;
  }
